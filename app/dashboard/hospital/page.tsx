...
import { Calendar, Clock, Users, FileText, Activity, Building2 } from "lucide-react"
...
export default async function HospitalDashboard() {
  ...
  // Verificar se o usuário é admin hospitalar
  ...
  // Buscar médicos ativos (simulando com perfis de médicos)
  const { data: activeDoctors } = await supabase.from("profiles").select("*").eq("user_type", "doctor")

  // Buscar total de pacientes
  const { data: totalPatients } = await supabase.from("profiles").select("id").eq("user_type", "patient")
  ...
  const { data: pendingExams } = await supabase
    ...
    .select(`
      *,
      ...
      doctor:profiles!exams_doctor_id_fkey(full_name)
    `)
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">Dashboard Hospital</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{activeDoctors?.length || 0}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{totalPatients?.length || 0}</div>
      ...
      <Tabs defaultValue="agenda" className="space-y-4">
        <TabsList>
          ...
          <TabsTrigger value="medicos">Equipe Médica</TabsTrigger>
                      ...
                      <div className="flex items-center space-x-4">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-500">
                            Dr(a). {appointment.doctor?.full_name || "N/A"} -{" "}
                            {appointment.specialty || "Consulta Geral"}
                          </p>
                        </div>
                      </div>
                      <div className="flex items-center space-x-2">
        ...
        <TabsContent value="medicos" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Médicos cadastrados no sistema</CardDescription>
            </CardHeader>
            <CardContent>
                ...
                <div className="space-y-4">
                  {activeDoctors.map((doctor) => (
                    <div key={doctor.id} className="flex items-center justify-between p-4 border rounded-lg">
                      <div>
                        <p className="font-medium">Dr(a). {doctor.full_name}</p>
                  ...
                  {pendingExams.map((exam) => (
                    <div key={exam.id} className="flex items-center justify-between p-4 border rounded-lg">
                      <div>
                        ...
                        <p className="text-sm text-gray-500">
                          Paciente: {exam.patient?.full_name || "N/A"} | Médico: Dr(a).{" "}
                          {exam.doctor?.full_name || "N/A"}
                        </p>
                        <p className="text-xs text-gray-400">
